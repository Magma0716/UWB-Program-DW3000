/*! ----------------------------------------------------------------------------
 *  @file    ss_AES_twr_initiator.c
 *  @brief   Single-sided two-way ranging (SS AES TWR) initiator example code
 *
 *           This is a simple code example which acts as the initiator in a SS TWR distance measurement exchange.
 *           In this example, data including time signature will be sent encrypted. This example is using a simple 802.15.4 standard data frame.
 *           This example is sending an encrypted data, it assumes that ss_AES_twr_resp.c data will be encrypted also.
 *           This application sends a "poll" frame (recording the TX time-stamp of the poll), after which it waits for a "response" message from the "DS TWR
 *           responder" example code (companion to this application) to complete the exchange. The response message contains the remote responder's time-stamps
...
mac_frame_802_15_4_format_t mac_frame = {
     ...
     *                         ASN in Nonce: 0x0 = frame counter is used to generate the nonce (CCM* nonce = SRC ADDR (8), Frame Counter (4) and Nonce Security
     *                         Level (1) - set to 0x7 above) This means that format of the AUX header is Security Control (1 octet) + Fame Counter (4 octets)
     *                         + Key Identifier (1 octet) = 6 octets
     */
...
int ss_aes_twr_initiator(void)
{
    ...
    while (1)
    {
        ...
        { /* Got response */
            ...
            if (memcmp(&rx_buffer[START_RECEIVE_DATA_LOCATION], &rx_resp_msg[START_RECEIVE_DATA_LOCATION], aes_job_rx.payload_len - START_RECEIVE_DATA_LOCATION)
                == 0)
            {
 ...
 *    6.8M data rate used (around 250 �s).
