...
if section == 0:
    ...
    if choose == 0: # 非加密
        ''' this is an old misktake data dictonary (dont use it)
        data = {
        ...
        data = {
            'Tags':     [1, 2, 2, 3, 3, 3, 4, 4, 4, 4],
            'Residual': [0.0164,    0.0171, 0.0197,    0.0517,0.0196, 0.0194,     0.0336, 0.0200, 0.0783, 0.0880],
            'Jitter':   [0.0336,    0.0485, 0.0332,    0.0724,0.0386, 0.0322,     0.0410, 0.0341, 0.1503, 0.0966],
            'Latency':  [2.29,      2.30, 2.29,        2.34,2.30, 2.33,           2.34, 2.35, 2.37, 2.31]
        }
    elif choose == 1: # AES加密
        data = {
            'Tags':     [1, 2, 2, 3, 3, 3, 4, 4, 4, 4],
