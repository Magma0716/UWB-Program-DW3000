...
class UWB_EKF:
    ...
    def update(self, distances, anchor_pos):
        for aid, z_meas in distances.items():
            ...
            y_error = z_meas - z_pred
            self.X = self.X + np.dot(K, y_error)
...
class UWBPositionSystem:
    def __init__(self):
        ...
        self.ax.set_title('UWB Tracking: Multi-Anchor Logic + EKF')

        # 圓圈與連線 (預建 Artist)
        self.circles = {aid: plt.Circle(pos, 0, color='#a9fbab', fill=False, lw=1.5, visible=False) 
                        for aid, pos in self.anchor_positions.items()}
        for c in self.circles.values(): self.ax.add_artist(c)
        
        self.lines = {k: self.ax.plot([], [], 'b-', alpha=0.6)[0] for k in ['L12', 'L23', 'L31']}
        
        # Anchor 靜態標記 (加上 zorder 確保不被覆蓋)
        self.anchor_dots = []
        for aid, pos in self.anchor_positions.items():
        ...
        self.tag_point, = self.ax.plot([], [], 'ro', markersize=10, label='EKF Filtered', zorder=11)
        self.raw_point, = self.ax.plot([], [], 'kx', markersize=7, alpha=0.4, label='Raw Meas.')
        self.tag_label = self.ax.text(0, 0, '', fontsize=10, fontweight='bold', color='red', zorder=12)
