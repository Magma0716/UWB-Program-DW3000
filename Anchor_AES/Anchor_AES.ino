/*! ----------------------------------------------------------------------------
 *  @file    ss_AES_twr_responder.c
 *  @brief   Single-sided two-way ranging (SS AES TWR) responder example code
 *
 ...
 *           This example is getting an encrypted data, its response will be encrypted also.
 *           This application waits for a "poll" message (recording the RX time-stamp of the poll) expected from the "SS TWR initiator"
 *           example code (companion to this application), and then sends a "response" message to complete the exchange.
 *           The response message contains all the time-stamps recorded by this application,
